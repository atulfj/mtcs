...
After using scipy.interpolate.griddata with method='linear', I noticed
that there are no nan values. Even with low levels of sampling such as 1
or 0.1 percent, nan values didn't appear. According to the scipy
documentation
...
To test this, I have written no_eight.py to try sampling without taking the 8 corner points
explicitly. After this, nan values started appearing (6892 nan at 1% sampling, 2534 nan at 3% sampling).
"""
import random
import argparse
import time
...
def srs(data, percent):
	...
	n_samples = int(total_points * (percent/100))
